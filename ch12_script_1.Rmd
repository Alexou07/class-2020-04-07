...
```{r}
x %>% 
  summarize(correlation_1 = cor(faculty, earnings)) %>% 
  pull(correlation_1)

x %>% 
  summarize(correlation_2 = cor(sat, earnings)) %>% 
  pull(correlation_2)
...
```{r}
x %>%
  ggplot(aes(earnings, faculty)) + geom_point() +
  geom_smooth(method = lm, se = 0, formula = y ~ x)

x %>%
  ggplot(aes(earnings, sat)) + geom_point() +
  geom_smooth(method = lm, se = 0, formula = y ~ x)
```
...
Unit = Universities.
Treatments = Decrease in SAT scores.
Outcomes = change in median earnings
Causal effect of the policy change would be is decreasing financial aid leading to a decrease in SAT scores directly causes a decrease in 10 year earnings.
the fundamental problem of causal inference = cannot assign on variable to both treatment and control groups.
To make a causal claim we must assume that there are no other confounding variables.
...
data <- tibble(sat = c(1400, 1300),
               faculty = c(50000, 40000))
